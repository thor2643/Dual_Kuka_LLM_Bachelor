...
class ObjectDetector(Node):
    def __init__(self):
        ...
        self.detector_srv = self.create_service(GetObjectInfo, 'get_object_info', self.callibrate_camera)
    ...
    def get_object_information(self, request, response, clustered = True):
        ...
        self.transformation_matrix = np.eye(4) # Initialize the transformation matrix
    ...
    def callibrate_camera(self, request, response):
    ...
    def show_cartesian_coordinates(self):
        def mouse_callback(event, x, y, flags, param):
            if event == cv2.EVENT_MOUSEMOVE:
                ...
                if coordinates is not None:
                    self.get_logger().info(f"Cartesian coordinates at ({x}, {y}): {coordinates}")
        ...
        while True:
            cv2.imshow("Image with Coordinates",self.color_image)
